    ...
    for set_dir in set_dirs:
        ...
        for card_file in price_files:
            ...
            if card_name in get_feature_selection_table():
                ...
                if card_name not in rl_predictors_map:
                    predictor = MTGO_Q_learner(set_dir, card_file, start)
                    predictor.learn()
                    card_name = os.path.splitext(card_file)[0]
                    rl_predictors_map[card_name] = predictor
                """ Q for possible buy action, having card = False """
                Q_value, price = rl_predictors_map[card_name].get_Q_prediction(now_date, False)
                data[card_name] = [price, Q_value[0]]


def get_investment_margin_list(now_date):
    margin_list = []
    for card_name, info in data.copy().items():
        price = info[0]
        Q_buying = info[1][0]
...
def manage_owned_cards(margin_list, now_date):
    for card_name, price_key in owned_cards.copy().items():
        ...
        Q_value, today_sell_price = rl_predictors_map[card_name].get_Q_prediction(now_date, True)
        ...
        for past_price, quantity in price_key.copy().items():
            ...
            if Q_selling > Q_no_action and (loss_percentage > BH_stoploss_threshold or - loss_percentage > BH_stopgain_threshold):
...
if __name__ == "__main__":
    with open(get_data_location() + "Simulation_FQI_30_30.txt", "w") as datafile:
